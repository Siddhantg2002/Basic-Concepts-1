...
// Strongly Connected Component :-  if we can reach from every vertex to every other vertex in a component 
// then it is called strongly connected component.
// Strongly Connected Graph:-if in a graph every vertex is reachable from every another vertex then 
...
// Step3:-Pop node one by one from stack and again do DFS traversal on modified graph keep popping nodes
//        each successful DFS traversal give 1 Strongly Connected Component.
